...
pub fn view_box_to_transform_with_clip(
    ...
    let ts = usvgr::Transform::from_row(sx, 0.0, 0.0, sy, tx, ty);

    (ts, clip)
}

/// Fits size into a viewbox.
pub fn fit_view_box(size: usvgr::Size, vb: &usvgr::ViewBox) -> usvgr::Size {
    let s = vb.rect.size();

    if vb.aspect.align == usvgr::Align::None {
        s
