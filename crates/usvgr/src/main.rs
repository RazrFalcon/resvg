...
USAGE:
  usvgr [OPTIONS] <in-svg> <out-svg>  # from file to file
  usvgr [OPTIONS] <in-svg> -c         # from file to stdout
  usvgr [OPTIONS] - <out-svg>         # from stdin to file
  usvgr [OPTIONS] - -c                # from stdin to stdout
...
fn process(args: Args) -> Result<(), String> {
    ...
    let tree =
        usvgr::Tree::from_data(&input_svg, &re_opt, &fontdb).map_err(|e| format!("{}", e))?;

    let xml_opt = usvgr::WriteOptions {
