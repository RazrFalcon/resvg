...
static GLOBAL_FONTDB: Lazy<std::sync::Mutex<usvgr::fontdb::Database>> = Lazy::new(|| {
...
fn resave_impl(name: &str, id_prefix: Option<String>, preserve_text: bool) {
    ...
    let tree = {
        ...
        usvgr::Tree::from_str(&input_svg, &opt, &fontdb).unwrap()
    };
    let mut xml_opt = usvgr::WriteOptions::default();
