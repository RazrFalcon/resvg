...
Note: the current version supports the `text` element,
but eventually text will be converted into paths.
To make SVG Micro even simpler.
...
* `fill-rule` = `evenodd`? +
  Default: nonzero
* `stroke` = <<color-type,<color> >> | <<func-iri-type,<FuncIRI> >>? +
...
_usvg_ will modify the `text` element and its children a lot.
The simplest `text` element like `<text>Text</text>` will be converted into:
...
* `clip-path` = <<func-iri-type,<FuncIRI> >>? +
  Available only inside the <<clipPath-element,clipPath>>.
* `text-rendering` = `optimizeSpeed | geometricPrecision` +
  Default: optimizeLegibility
* `transform` = <<transform-type,<transform> >>?

*Text chunk attributes:*
...
* `stroke-opacity` = <<opacity-type,<opacity> >>? +
  Default: 1
* `font-family` = <<string-type,<string> >>
* `font-size` = <<positive-number-type,<positive-number> >>
* `font-style` = `italic | oblique`? +
  Default: normal
* `font-variant` = `small-caps`? +
  Default: normal
* `font-weight` = `100 | 200 | 300 | 500 | 600 | 700 | 800 | 900`? +
  Default: 400
* `font-stretch` = `wider | narrower | ultra-condensed | extra-condensed` +
  `| condensed | semi-condensed | semi-expanded | expanded` +
  `| extra-expanded | ultra-expanded` ? +
  Default: normal
* `baseline-shift` = <<number-type,<number> >>? +
  Default: baseline
* `letter-spacing` = <<number-type,<number> >>? +
  Default: normal
* `word-spacing` = <<number-type,<number> >>? +
  Default: normal
* `visibility` = `hidden | collapse`? +
  Default: visible
...
* `k1` = <<compositing-coefficient-type,<compositing-coefficient> >> +
  Present only when `operator=arithmetic`.
* `k2` = <<compositing-coefficient-type,<compositing-coefficient> >> +
  Present only when `operator=arithmetic`.
* `k3` = <<compositing-coefficient-type,<compositing-coefficient> >> +
  Present only when `operator=arithmetic`.
* `k4` = <<compositing-coefficient-type,<compositing-coefficient> >> +
  Present only when `operator=arithmetic`.
* <<Filter primitive attributes>>
...
*<positive-number>* - A positive, non-zero real <<number-type,number>>. +
...
[[compositing-coefficient-type]]

*<compositing-coefficient>* - A real <<number-type,number>> in a 0..1 range. +
`compositing-coefficient ::= positive-number`
