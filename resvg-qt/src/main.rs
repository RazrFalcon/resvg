...
fn render_svg(args: Args, tree: &usvg::Tree, out_png: &path::Path) -> Result<(), String> {
    ...
    match img {
        Some(img) => {
            timed!(args, "Saving", img.save(out_png.to_str().unwrap()));
            Ok(())
        }
        None => {
...
const HELP: &str = "\
resvg-qt is an SVG rendering application.

USAGE:
    resvg-qt [OPTIONS] <in-svg> <out-png>

    resvg-qt in.svg out.png
    resvg-qt -z 4 in.svg out.png
    resvg-qt --query-all in.svg
