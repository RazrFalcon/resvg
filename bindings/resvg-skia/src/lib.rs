...
impl Canvas {
    ...
    pub fn set_matrix(&mut self, matrix: &Matrix) {
        unsafe {
            ffi::skiac_canvas_set_matrix(self.0, matrix.0);
        }
    }

    pub fn concat(&mut self, matrix: &Matrix) {
        unsafe {
            ffi::skiac_canvas_concat(self.0, matrix.0);
        }
    }
    ...
    pub fn reset_matrix(&mut self) {
        unsafe {
            ffi::skiac_canvas_reset_matrix(self.0);
        }
    }
...
impl Shader {
    pub fn new_linear_gradient(grad: LinearGradient) -> Shader {
        let points = [
            ffi::skia_point {
                x: grad.start_point.0 as f32,
                y: grad.start_point.1 as f32,
            },
            ffi::skia_point {
                x: grad.end_point.0 as f32,
                y: grad.end_point.1 as f32,
            },
        ];
    ...
    pub fn new_from_surface_image(surface: &Surface, matrix: Matrix) -> Shader {
        unsafe {
            Shader(ffi::skiac_shader_make_from_surface_image(
                surface.ptr,
                matrix.0,
            ))
        }
